...
// Throttle is a middleware that limits the number of current requests being processed simultaneously for all users.
// Note: Throttle is not a rate limiter for each user, instead it simply sets a ceiling on the number of
// current requests processed from the point at which the Throttle middleware is installed.
func Throttle(limit int) func(http.Handler) http.Handler {
	return ThrottleWithOpts(ThrottleOpts{Limit: limit, BacklogTimeout: defaultBacklogTimeout})
}

// ThrottleBacklog is a middleware that limits the number of pending requests at
// a time and provides a backlog to store the final number of pending requests.
...
func ThrottleWithOpts(opts ThrottleOpts) func(http.Handler) http.Handler {
	if opts.Limit < 1 {
		panic("gor/middleware: Throttle expects limit > 0")
	}

	if opts.BacklogLimit < 0 {
		panic("gor/middleware: Throttle expects backlogLimit to be positive")
	}
